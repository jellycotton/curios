...
function createBaseOverlay(position) {
  ...
  const screenWidth = window.innerWidth;
  const screenHeight = window.innerHeight;
  const overlayWidth = Math.min(350, screenWidth - 40); // 最大350px、画面幅-40px
  const overlayMaxHeight = Math.min(screenHeight * 0.8, 600, screenHeight - 40); // 画面の80%、最大600px、または画面高-40px
  
  // ★ スマート配置ロジック
  let top = position.y + PREFERRED_OFFSET;
  let left = position.x + PREFERRED_OFFSET;
  let placement = 'bottom-right'; // デフォルト配置方向
  ...
  if (spaceRight >= overlayWidth + SCREEN_MARGIN) {
    // 右側に十分なスペースがある
    left = position.x + PREFERRED_OFFSET;
    placement = placement.replace('right', 'right');
  } else if (spaceLeft >= overlayWidth + SCREEN_MARGIN) {
    // 左側に十分なスペースがある
    left = position.x - overlayWidth - PREFERRED_OFFSET;
  ...
  // ★ 改善された縦方向の配置決定
  if (spaceBelow >= MIN_OVERLAY_HEIGHT + PREFERRED_OFFSET) {
    // 下側に最小限必要なスペースがある場合
    top = position.y + PREFERRED_OFFSET;
    placement = placement.replace('bottom', 'bottom');
    
    // もしオーバーレイが画面下端を超える場合のみ高さ調整
    if (top + overlayMaxHeight > screenHeight - SCREEN_MARGIN) {
      const availableHeight = screenHeight - top - SCREEN_MARGIN;
      if (availableHeight >= MIN_OVERLAY_HEIGHT) {
        overlay.style.maxHeight = `${availableHeight}px`;
      } else {
        // 高さが足りない場合のみ上に移動
        top = screenHeight - overlayMaxHeight - SCREEN_MARGIN;
      }
    }
  } else if (spaceAbove >= MIN_OVERLAY_HEIGHT + PREFERRED_OFFSET) {
    // 上側に最小限必要なスペースがある場合
    if (spaceAbove >= overlayMaxHeight + PREFERRED_OFFSET) {
      // 完全に上に配置できる場合
      top = position.y - overlayMaxHeight - PREFERRED_OFFSET;
    } else {
      // 部分的に上に配置し、高さを調整
      const availableHeight = spaceAbove - PREFERRED_OFFSET - SCREEN_MARGIN;
      top = position.y - availableHeight - PREFERRED_OFFSET;
      overlay.style.maxHeight = `${Math.max(MIN_OVERLAY_HEIGHT, availableHeight)}px`;
    }
    ...
    if (spaceBelow > spaceAbove) {
      // 下側により多くスペースがある場合
      top = position.y + PREFERRED_OFFSET;
      const availableHeight = spaceBelow - PREFERRED_OFFSET - SCREEN_MARGIN;
      overlay.style.maxHeight = `${Math.max(MIN_OVERLAY_HEIGHT, availableHeight)}px`;
    } else {
      // 上側により多くスペースがある場合、ただし押した位置から極端に離さない
      const maxUpwardDistance = Math.min(spaceAbove - SCREEN_MARGIN, MAX_UPWARD_DISTANCE);
      top = Math.max(SCREEN_MARGIN, position.y - maxUpwardDistance);
      const availableHeight = position.y - top + spaceBelow - PREFERRED_OFFSET;
      overlay.style.maxHeight = `${Math.max(MIN_OVERLAY_HEIGHT, Math.min(availableHeight, overlayMaxHeight))}px`;
    }
  }
  
  // 最終的な境界チェックと調整
  left = Math.max(SCREEN_MARGIN, Math.min(left, screenWidth - overlayWidth - SCREEN_MARGIN));
  top = Math.max(SCREEN_MARGIN, Math.min(top, screenHeight - MIN_OVERLAY_HEIGHT - SCREEN_MARGIN)); // 最小必要高さを確保
  
  console.log(`Overlay placement: ${placement}, mouse: (${position.x}, ${position.y}), overlay: (${left}, ${top}), size: ${overlayWidth}x${overlayMaxHeight}, spaces: ↑${spaceAbove} ↓${spaceBelow} ←${spaceLeft} →${spaceRight}`);
